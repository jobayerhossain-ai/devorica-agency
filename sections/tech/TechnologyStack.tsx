        ...
        <SectionLayout background="dark">
            ...
            >
                {TECH_STACK.map((tech, index) => (
                    <div
                        key={index}
                        className="group flex flex-col items-center justify-center p-8 bg-card border border-border rounded-2xl relative overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:border-accent/50 cursor-default"
                    >
                        {/* Hover Ambient Glow */}
                        ...
                        <span className="text-foreground-muted font-medium group-hover:text-foreground transition-colors duration-300 relative z-10 text-center text-sm md:text-base">
                            {tech.name}
                        </span>
                    </div>
                ))}
