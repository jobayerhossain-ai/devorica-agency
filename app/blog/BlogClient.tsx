        ...
        <div className="bg-[#0B0B0B]">
            ...
            <section className="relative pt-40 pb-32 overflow-hidden border-b border-[#1F2937]">
                ...
                <div className="max-w-[1280px] mx-auto px-6 text-center relative z-10">
                    ...
                    >
                        <div className="inline-flex items-center gap-2 mb-8 px-4 py-1.5 rounded-full border border-[#1F2937] bg-[#0F172A] text-accent text-sm font-semibold tracking-wider uppercase">
                            <span className="w-1.5 h-1.5 rounded-full bg-accent" />
                            Insights & Engineering
                        </div>
                        <h1 className="text-6xl md:text-7xl lg:text-8xl font-bold font-heading text-white mb-8 tracking-tighter leading-tight">
                ...
                <div className="max-w-[1280px] mx-auto px-6">
                    ...
                    >
                        {currentPosts.map((post) => (
                            ...
                            >
                                <Link href={post.href} className="flex flex-col h-full bg-[#0F172A] rounded-xl border border-[#1F2937] overflow-hidden group-hover:border-accent/20 group-hover:shadow-2xl group-hover:shadow-accent/5 transition-all duration-500">
                                    {/* Article Image Preview */}
                                    <div className="relative w-full aspect-[16/10] overflow-hidden bg-[#111827]">
                                        <Image
                                            src={post.image}
                                            alt={post.title}
                                            fill
                                        ...
                                        <div className="absolute top-5 left-5 z-20">
                                            <span className="px-3 py-1 text-[10px] font-bold tracking-widest uppercase rounded-full border border-white/10 bg-black/40 backdrop-blur-md text-white">
                                                {post.category}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-8 md:p-10 flex flex-col flex-1">
                                        {/* Metadata Row */}
                                        <div className="flex items-center gap-6 text-[11px] font-bold uppercase tracking-widest text-[#9CA3AF] mb-6">
                                        ...
                                        <h3 className="text-2xl font-bold font-heading text-white mb-4 group-hover:text-accent leading-tight transition-colors duration-300">
                                            {post.title}
                                        </h3>

                                        <p className="text-[#9CA3AF] text-base leading-relaxed mb-8 flex-1">
                                            {post.excerpt}
                                        </p>

                                        {/* Footer Row */}
                                        <div className="flex items-center justify-between pt-8 border-t border-[#1F2937] mt-auto">
                                            <div className="flex flex-col">
                                                <span className="text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1">Author</span>
                                                <span className="text-sm font-bold text-white tracking-tight">{post.author}</span>
                                            </div>
                                            <div className="w-10 h-10 rounded-full border border-[#1F2937] flex items-center justify-center text-white group-hover:border-accent/40 group-hover:bg-accent group-hover:text-black transition-all duration-500">
                    ...
                    {totalPages > 1 && (
                        ...
                        >
                            <Button
                                ...
                                className="w-12 h-12 p-0 rounded-xl border-[#1F2937] bg-[#0F172A] hover:bg-accent hover:text-black hover:border-accent disabled:opacity-20 transition-all duration-300"
                            >
                            ...
                            {[...Array(totalPages)].map((_, i) => (
                                <button
                                    ...
                                    className={`w-12 h-12 rounded-xl font-bold tracking-tight transition-all duration-300 ${currentPage === i + 1
                                        ...
                                        : "bg-[#0F172A] text-white border border-[#1F2937] hover:border-accent/50"
                                        }`}
                                >
                                    {i + 1}
                                </button>
