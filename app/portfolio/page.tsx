        ...
        <div className="bg-surface-alt">
            ...
            <section className="relative pt-40 pb-32 overflow-hidden">
                ...
                <div className="max-w-[1280px] mx-auto px-6 text-center relative z-10">
                    ...
                    >
                        <div className="inline-flex items-center gap-2 mb-8 px-5 py-2 rounded-full border border-border bg-card-bg">
                            <span className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse" />
                            <span className="text-sm font-semibold text-subtext tracking-wider uppercase">
                                Proven Results
                            </span>
                        </div>
                        <h1 className="text-6xl md:text-7xl lg:text-8xl font-bold font-heading text-heading mb-8 tracking-tighter leading-[1.1]">
                        ...
                        {PORTFOLIO_PROJECTS.map((project, index) => (
                            ...
                            >
                                <Link href={project.href} className="flex-1 flex flex-col h-full">
                                    ...
                                    <div className="relative w-full aspect-[16/10] rounded-xl overflow-hidden mb-10 border border-border bg-card-bg shadow-lg group-hover:shadow-accent/5 group-hover:border-accent/20 transition-all duration-500">
                                        <Image
                                            src={project.image}
                                            alt={project.title}
                                            fill
                                    ...
                                    <div className="flex flex-col gap-5 flex-1 px-2">
                                        <div className="flex flex-col gap-3">
                                            <h3 className="text-3xl font-bold font-heading text-heading tracking-tight group-hover:text-accent transition-colors duration-300">
                                                {project.title}
                                            </h3>
                                        ...
                                        <div className="flex flex-wrap gap-2.5 mt-auto pt-4">
                                            {project.tech.map((t, i) => (
                                                <span
                                                    key={i}
                                                    className="px-3 py-1 text-xs font-semibold tracking-wide uppercase rounded-full border border-border bg-card-bg text-subtext group-hover:border-accent/20 group-hover:text-accent/80 transition-all duration-300"
                                                >
                                                    {t}
                                                </span>
                                            ))}
